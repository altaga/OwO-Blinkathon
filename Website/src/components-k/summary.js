...
import { Collapse, Card, CardBody, CardHeader, Button, Row, Col, Input } from "reactstrap"
...
class Summary extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.account_reducer.result.data.status === "ACT") {
        ...
        if (this.props.account_reducer.result.data.status === "ACT") {
            ...
            unirest('GET', 'https://e9wzhv9k7d.execute-api.us-east-1.amazonaws.com/get-transactions-ewallet')
                ...
                .end((res) => {
                    ...
                    let __this = _this
                    unirest('GET', 'https://e9wzhv9k7d.execute-api.us-east-1.amazonaws.com/get-source-from-transaction')
                        .headers({
                            'ewallet': _this.props.account_reducer.result.data.id,
                            'trans': JSON.parse(res.raw_body).data[JSON.parse(res.raw_body).data.length - 1].id
                        })
                        .end((res) => {
                            if (res.error) throw new Error(res.error);
                            __this.setState({
                                parentAccount: JSON.parse(res.raw_body).data.action_data.source_ewallet_id
                            })
                        });
                });
        }
    ...
    addToAccount() {
        this.setState({
            ...
            unirest('GET', 'https://e9wzhv9k7d.execute-api.us-east-1.amazonaws.com/put-request')
                .headers({
                    'account': this.props.account_reducer.result.data.id,
